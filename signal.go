...
func handleSignal() {
	// 创建了一个 os.Signal 类型的通道变量, 长度为 1 (每次受到 signal 都会及时处理, 所以设置长度为 1, 节省了空间)
	sigRecv1 := make(chan os.Signal, 1)
	// 设置两个需要自行处理的 signal (SIGINT, SIGQUIT)
	sigs1 := []os.Signal{syscall.SIGINT, syscall.SIGQUIT}
	fmt.Printf("Set notification for %s... [sigRecv1]\n", sigs1)
	// 设置一个信号触发器，当指定信号发生时发送到 sigRecv1
	// sigRecv1 必须是一个发送通道类型, 只能向它发生 os.Signal 的值, Notify 的可变参数可以是任意多个 signal
	signal.Notify(sigRecv1, sigs1...)

	// 创建了一个 os.Signal 类型的通道变量,, 长度为 1 (每次受到 signal 都会及时处理, 所以设置长度为 1, 节省了空间)
	sigRecv2 := make(chan os.Signal, 1)
	// 设置两个需要自行处理的 signal (SIGQUIT)
	sigs2 := []os.Signal{syscall.SIGQUIT}
	fmt.Printf("Set notification for %s... [sigRecv2]\n", sigs2)
	// 设置一个信号触发器，当指定信号发生时发送到 sigRecv1
	// sigRecv2 必须是一个发送通道类型, 只能向它发生 os.Signal 的值, Notify 的可变参数可以是任意多个 signal
	signal.Notify(sigRecv2, sigs2...)

	// 开启两个 goroutine 并发接受信号
	// 因为需要等待两个协程都完成才可以退出，所以需要用到 sync.WaitGroup
	var wg sync.WaitGroup
	// 设置等待协程数
	wg.Add(2)
	go func() {
		// 阻塞并等待 sigRecv1 受到 signal
